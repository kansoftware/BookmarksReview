...
class LoggerManager:
    ...
    def add_custom_handler(self, handler: logging.Handler) -> None:
        ...
        if self._root_logger:
            ...
            logger = self.get_logger(__name__)
            logger.debug(
                f"Добавлен пользовательский обработчик: {type(handler).__name__}"
            )
...
def log_function_call(
    func_name: str, args: tuple[Any, ...] = (), kwargs: Optional[dict[str, Any]] = None
) -> None:
    """
...
def log_performance(func_name: str, duration: float, details: str = "") -> None:
    ...
    logger.info(
        f"Производительность: {func_name} выполнена за {duration:.2f}с{details_str}"
    )
