...
class ConfigManager:
    ...
    def __init__(self, env_path: Optional[str] = None):
        ...
        logger.debug(f"Загружена конфигурация: output_dir={self.config.output_dir}, "
    ...
    def _validate_config(self):
        ...
        if validation_errors:
            logger.error(f"Валидация конфигурации не пройдена: {len(validation_errors)} ошибок")
            raise ValueError(f"Ошибки валидации конфигурации: {'; '.join(validation_errors)}")
...
def setup_logging(log_level: str = "INFO") -> None:
    ...
    """
    logger.warning("Используется устаревшая функция setup_logging из config.py. "
