...
class ProgressManager:
    ...
    def __init__(self, output_dir: str, bookmarks_file: str, config_hash: str):
        ...
        """
        log_function_call("ProgressManager.__init__", (output_dir, bookmarks_file), {"config_hash": config_hash})
        ...
        try:
            with self._lock:
                ...
                if data.get('version') != '1.0':
                    logger.warning(f"Несовместимая версия прогресса: {data.get('version')}")
                    return False
                
                # Проверка конфигурации
                if data.get('config_hash') != self.config_hash:
                    logger.warning("Хеш конфигурации не совпадает, прогресс несовместим")
                    return False
                ...
                self.processed_bookmarks = [
                    ProcessedBookmark(**item) 
                    for item in data.get('processed_urls', [])
                ]
                
                # Загрузка неудачных закладок
                self.failed_bookmarks = [
                    FailedBookmark(**item) 
                    for item in data.get('failed_urls', [])
                ]
                ...
                log_performance("ProgressManager.load_progress", duration, 
                              f"processed={len(self.processed_bookmarks)}, failed={len(self.failed_bookmarks)}")
                
                logger.info(f"Прогресс загружен: {len(self.processed_bookmarks)} обработано, "
        ...
        try:
            with self._lock:
                ...
                data = {
                    ...
                    'processed_urls': [asdict(item) for item in self.processed_bookmarks],
                    'failed_urls': [asdict(item) for item in self.failed_bookmarks],
                }
                
                # Добавляем текущую позицию если есть
                if self.current_position:
                    data['current_position'] = asdict(self.current_position)
                ...
                duration = time.time() - start_time
                log_performance("ProgressManager.save_progress", duration, 
                              f"processed={len(self.processed_bookmarks)}, failed={len(self.failed_bookmarks)}")
                
                logger.debug(f"Прогресс сохранен: {len(self.processed_bookmarks)} обработано, "
                           f"{len(self.failed_bookmarks)} с ошибками")
    ...
    def add_processed_bookmark(self, bookmark: Bookmark, file_path: str, folder_path: List[str]) -> None:
        """
        ...
        """
        log_function_call("ProgressManager.add_processed_bookmark", (bookmark.title,), 
    ...
    def add_failed_bookmark(self, bookmark: Bookmark, error: str, folder_path: List[str]) -> None:
        """
        ...
        Аргументы:
            ...
            folder_path: Путь в иерархии папок
        """
        log_function_call("ProgressManager.add_failed_bookmark", (bookmark.title,), 
    ...
    def update_current_position(self, folder_path: List[str], bookmark_index: int, total_in_folder: int) -> None:
        """
        ...
        """
        log_function_call("ProgressManager.update_current_position", 
                         ("/".join(folder_path),), {"bookmark_index": bookmark_index})
    ...
    def get_resume_position(self) -> Optional[Tuple[List[str], int]]:
        ...
        with self._lock:
            if self.current_position:
                return (self.current_position.folder_path, self.current_position.bookmark_index)
            return None
...
def calculate_config_hash(config) -> str:
    """
