...
2. Проверяется совместимость конфигурации через хеш
3. Выбираются только те URL, которые были отмечены как неудачные в предыдущих сеансах
4. Выполняется повторная попытка обработки только этих URL
5. Успешно обработанные URL перемещаются из списка неудачных в список обработанных
