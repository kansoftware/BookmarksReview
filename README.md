...
### Обработка длинных имен файлов и папок
- Автоматическое ограничение длины имен файлов и папок с учетом ограничений файловой системы
- Учет родительского пути при определении максимальной длины имени файла для предотвращения ошибки "File name too long"
- Резервирование места для расширения файла (например, .md) при ограничении длины
- Поддержка Unicode-символов в именах файлов и папок
...
### Перепроверка ошибочных URL
При использовании флага `--check-error`:
1. Загружается сохраненный прогресс из `bookmarks_export/progress.json` (или указанного файла через `--progress-file`)
2. Проверяется совместимость конфигурации через хеш
3. Выбираются только те URL, которые были отмечены как неудачные в предыдущих сеансах (в `failed_urls`) или которые были обработаны с ошибкой (в `processed_urls` с заполненным полем `error`)
4. Выполняется повторная попытка обработки только этих URL
5. Успешно обработанные URL перемещаются из списка неудачных или из списка обработанных с ошибкой в список обработанных без ошибки
6. URL, которые снова завершились с ошибкой, остаются в списке неудачных